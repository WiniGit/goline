...
}
{
        ...
        switch ($($('.f12-container .tab.selected')).data('tab')) {
            case 1:
                if (selected_list.length > 0) {
                    $('.f12-container .select-api-container .show-select-request-button').addClass('active');
                } else {
                    $('.f12-container .select-api-container .show-select-request-button').removeClass('active');
                }
                let request = f12_getRequest();
                $('.f12-container .select-api-container .selected-request').val(request?.Url ?? "");
                // f12_update_listOutputRow();
                break;
            case 2:
                $('.f12-container .select-api-container .show-select-request-button').addClass('active');
                $('.f12-container .select-api-container .selected-request').val(F12Container.api_input?.Url ?? "");
                // ! add function update list input UI
                f12_update_listInputRow();
                break;
        }

        if (selected_list.length == 1 && $('.f12-container').css('display') != 'none') {
            let parent = selected_list[0].Level == 1 ? selected_list[0] : wbase_list.find(e => e.GID == selected_list[0].ListID.split(',')[1]);
            let list_tab = wbase_list.filter(e => e.PrototypeID == parent.GID);
            let tab = '';
            let tabview = '';
            if (list_tab.length > 0) {
                for (let item of list_tab) {
                    let prototype_item = item.JsonEventItem.find(e => e.Name == "Prototype");
                    let parent_item = item.Level == 1 ? item : wbase_list.find(e => e.GID == item.ListID.split(',')[1]);
                    if (prototype_item != null && prototype_item.Data.length > 0) {
                        tab += '<div data-tab="' + `${4 + list_tab.indexOf(item)}` + '" data-id="' + `${item.GID}` + '" class="prototype-tab tab semibold11 text-disable center">' + parent_item.Name + '</div>';
                        tabview +=
                            '<div data-tab="' + `${4 + list_tab.indexOf(item)}` + '" class="prototype-view  view col ">' +
    ...
    let wdata_id;
    $('body').on('click', '.prototype-view .row-value:not(.table-header)', function (ev) {
        ev.stopPropagation();
        update_UI_dataPrototype();
        $('.f12-select-data-popup').css('width', $(this).width() + 24);
        $('.f12-select-data-popup').css('top', $(this).parent().offset().top - $('.f12-container').offset().top + 36);
    ...
    $('body').on('click', '.f12-container .tab', function (ev) {
        ...
        $('.f12-container .tab-bar-view .view[data-tab="' + $(this).data('tab') + '"]').addClass('selected');
    ...
    // TODO: Select request
    $('body').on('click', '.f12-container .select-api-container .show-select-request-button', function (ev) {
        ev.stopPropagation();
        if ($(this).hasClass('active')) {
            $('.f12-container .select-api-container .select-request-popup').css('top', 92);
    ...
    function f12_create_requestRow(item) {
        let request_row =
            '<div data-id="' + `${item.GID}` + '" class="request-row col text-white ' + `${item.GID == F12Container.selected_id ? "selected" : ""}` + '">' +
            '   <div class="regular0 text-subtitle-white">' + `${CollectionDA.getElementByID(item.CollectionID).Name}/ ${item.Name}` + '</div>' +
            '   <div class="regular11 text-white">' + `${item.Url}` + '</div>' +
            '</div>';
        return request_row;
    }

    function f12_update_listRequest() {
        let list_request = '<div class="request-row row regular11 text-white">None</div>';
        for (let i = 0; i < RequestDA.list.length; i++) {
            list_request += f12_create_requestRow(RequestDA.list[i]);
        }

        $('.f12-container .select-api-container .select-request-popup').html(list_request);
        $('.f12-container .select-api-container .select-request-popup').css('display', 'flex');
    }
    ...
    $('body').on('mouseover', '.select-request-popup .request-row', function (ev) {
    ...
    window.onclick = function (ev) {
        if (!$(ev.target).is('.select-request-popup') || $('.select-request-popup').has(ev.target).length === 1) {
            $('.select-request-popup').hide();
        }
    }
    ...
    function f12_create_outputRow(item, index, space) {
        let output = RequestDA.selected?.outputApiItem.find((e) => e.GID == item.AttributesItem.OutputID);
        ...
        let dataType = output?.DataType == 1 ? "Field" : output?.DataType == 2 ? "List" : output?.DataType == 3 ? "Object" : item.AttributesItem?.Content ? "Field" : "";
        let output_row =
            '<div data-id="' + `${item.GID}` + '" class="f12-output-row row f12-setting-row">' +
            '    <div class="row regular1 text-title index center">' + index + '</div>' +

            '    <div class="row name">' +
            '        <input class="regular1 text-title" placeholder="Add a name field" value="' + `${item.AttributesItem.NameField}` + '" type="text" style="padding-left:' + `${16 + space}px;` + '"/>' +
            '    </div>' +

            '    <div class="row data-type">' +
            '        <input class="regular1 text-title" placeholder="-- Data type --" value="' + dataType + '" type="text" readonly/>' +
            '    </div>' +

            '    <div class="row init-value">' +
            '        <input class="regular1 text-title" placeholder="Init value" value="' + `${item.AttributesItem?.Content ?? ""}` + '" type="text" />' +
            '    </div>' +

            '    <div class="row output-value">' +
            '        <input class="regular1 text-title" placeholder="-- Output --" value="' + `${output?.Name ?? ""}` + '" type="text" readonly/>' +
            '    </div>' +

            '    <div class="row calculate">' +
            '        <input class="regular1 text-title" placeholder="Add calculate" value="' + `${output?.Calculate ?? ""}` + '" type="text" />' +
            '    </div>' +
            '</div>';
    ...
    function f12_update_listOutputRow() {
        let list_field = '';
        for (let i = 0; i < selected_list.length; i++) {
            list_field += f12_create_outputRow(selected_list[i], i + 1, 0);
        }
    ...
    function f12_create_outputRowPopup(item, space) {
        let row = '<div data-id=' + item.GID + ' class="option-tile row text-white regular11" style="padding-left: ' + `${12 + space}px` + '">' + `${item.NameField}` + '</div>';
        if (item.DataType != 1) {
    ...
    function f12_create_outputPopup() {
        let list_output = '<div class="option-tile row text-white regular11">None</div>';
        if (RequestDA.selected) {
            let list = RequestDA.selected.outputApiItem.filter(e => e.ParentID == null);
            for (let item of list) {
    ...
    $('body').on('click', '.output-value', function (ev) {
        ...
        if (!$(this).parent().hasClass('row-header')) {
            ...
            $('.list-output-popup').css('top', $(this).offset().top - $('.f12-container').offset().top + 33);
    ...
    $('body').on('click', '.f12-output-row>*', function (ev) {
        $('.wbaseItem-value').removeClass('selected');
        $('.wbaseItem-value[id="' + $(this).parent().data('id') + '"]').addClass('selected');
    ...
    function f12_create_inputRow(item) {
        ...
        let input_row =
            '<div data-id="' + `${item.GID}` + '" class="f12-input-row row f12-setting-row">' +
            '    <div class="row regular1 text-title index">' + `${F12Container.api_input.inputApiItem.indexOf(item) + 1}` + '</div>' +
            '    <div class="row input-value">' +
            '        <input class="regular1 text-title" value="' + `${item.Name}` + '" type="text" readonly>' +
            '    </div>' +

            '    <div class="row wbase">' +
            '        <input class="regular1 text-title" value="' + `${wbase?.AttributesItem?.NameField ?? ""}` + '" placeholder="-- Select item --" type="text" readonly>' +
            '    </div>' +

            '    <div class="row calculate">' +
            '        <input class="regular1 text-title" value="' + `${item?.Calculate ?? ""}` + '" placeholder="Add calculate" type="text">' +
            '    </div>' +
            '</div>';
    ...
    function f12_create_wbaseRowPopup(item, space) {
        let row = '<div data-id=' + item.GID + ' class="option-tile row text-white regular11" style="padding-left: ' + `${12 + space}px` + '">' + `${item.AttributesItem?.NameField.length > 0 ? item.AttributesItem?.NameField : item.Name}` + '</div>';
    ...
    function f12_create_wbasePopup() {
        let list_wbase = '<div class="option-tile row text-white regular11">None</div>';
        if (selected_list.length > 0) {
            for (let i = 0; i < selected_list.length; i++) {
    ...
    $('body').on('click', '.f12-input-row .wbase', function (ev) {
        ...
        if (!$(this).parent().hasClass('row-header')) {
            ...
            $('.list-wbase-popup').css('top', $(this).offset().top - $('.f12-container').offset().top + 33);
    ...
    function create_UI_dataRow(item = selected_list[0], index = '', space = 0) {
        let data_item = wbase_list.find(e => e.GID == $('.prototype-tab.tab.selected').data('id'));
        let prototype_item = data_item?.JsonEventItem.find(e => e.Name == "Prototype");
        let data = prototype_item?.Data.find(e => e.ListID?.some(el => el?.toLowerCase() == item.GID));

        let row =
            '<div  data-id="' + item.GID + '" class="table-row row">' +
            '    <div class="table-cell regular11 row-index center">' + index + '</div>' +
            '    <div class="table-cell regular11 row-name">' +
            '        <input type="text"  value="' + item.Name + '" placeholder="Select map data item" readonly />' +
            '    </div>' +
            '    <div class="table-cell regular11 row-value">' +
            '        <input type="text"  value="' + `${data?.Key ?? ""}` + '" placeholder="Select data" readonly />' +
            '    </div>' +
    ...
    function update_UI_dataPrototype() {
        ...
        for (let item of prototype_item.Data) {
            list_data += '<div data-id="' + item.ID + '" class="data-option regular11 text-white">' + `${item?.Key ?? "Untitled"}` + '</div>';
        }
    ...
    $('body').on('click', '.f12-select-data-popup .data-option', function (ev) {
        ...
        if (item) {
            ...
            let update_dataItem = prototype_item?.Data.find(e => e.ListID?.some(el => el?.toLowerCase() == wdata_id));
