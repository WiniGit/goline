...
async function initComponents (item, list, initListener = true) {
    ...
    if (item.CateID == EnumCate.variant) {
      ...
      for (let property of item.PropertyItems) {
        property.BasePropertyItems = property.BasePropertyItems.filter(e =>
          item.ListChildID.some(id => id === e.BaseID)
        )
      }
    } else {
      let listBaseProperty = PropertyDA.list.map(e => e.BasePropertyItems)
      if (listBaseProperty.length > 0) {
        item.BasePropertyItems = listBaseProperty
          .reduce((a, b) => a.concat(b))
          .filter(e => e.BaseID === item.GID)
      } else {
  ...
  switch (item.CateID) {
    ...
    case EnumCate.tree:
      if (item.AttributesItem.Content != '')
        item.TreeData = JSON.parse(item.AttributesItem.Content)
      createTreeHTML(item, list)
      break
    case EnumCate.table:
      if (item.AttributesItem.Content != '')
        item.TableRows = JSON.parse(item.AttributesItem.Content)
    ...
    case EnumCate.carousel:
      if (item.AttributesItem.Content != '')
        item.CarouselData = JSON.parse(item.AttributesItem.Content)
  ...
  if (item.ListClassName) {
    item.ListClassName.split(' ').forEach(clssName =>
      $(item.value).addClass(clssName)
    )
  }
  //
  if (!item.build) {
...
async function updateComponentContent (item) {
  switch (item.CateID) {
    case EnumCate.rectangle:
      item.value.style.backgroundImage = `url(${
        urlImg + item.StyleItem.DecorationItem.replaceAll(' ', '%20')
      })`
      break
    case EnumCate.text:
      ...
      break
    case EnumCate.radio_button:
      item.value.querySelector(':scope > input').value =
      ...
      item.value.querySelector(':scope > input').onchange = function (e) {
        ...
        if (this.checked) {
          ...
          item.value.querySelector(':scope > .checkmark').style.display =
            'block'
      ...
      break
    case EnumCate.textformfield:
      item.value.querySelector('.textfield > input').value =
        item.AttributesItem.Content
      ...
      let input = item.value.querySelector(
        `.textfield > input[id="${item.GID}"`
      )
      ...
      input.onfocus = function (e) {
        ...
        if ($(this.parentElement).find('label')) {
          ...
          if (item.JsonEventItem) {
            let eventFocus = item.JsonEventItem.find(
              evt => evt.Name === 'State'
            )?.ListState?.find(state => state.Type === ComponentState.focus)
            if (
              eventFocus &&
              eventFocus.BorderSkinID &&
              eventFocus.BorderSkinID != ''
            ) {
              let borderSkin = BorderDA.list.find(
                skin => skin.GID === eventFocus.BorderSkinID
              )
      ...
      break
    case EnumCate.w_switch:
      item.value.querySelector(':scope > input').checked =
        item.AttributesItem.Content === 'true'
      item.value.querySelector(':scope > input').onchange = function (e) {
        e.stopImmediatePropagation()
        e.stopPropagation()
        if (item) {
...
function initElement (wbaseHTML) {
  switch (parseInt(wbaseHTML.getAttribute('cateid'))) {
    ...
    case EnumCate.tree:
      ...
      wbaseHTML
        .querySelectorAll('.children-value > .w-check-box')
        .forEach(chbox => $(chbox.querySelector('input')).trigger('change'))
      break
    default:
      break
  }
...
function initSkinWbase (item) {
  ...
  if (item.StyleItem.DecorationItem?.ColorID) {
    ...
    let colorSkin = (isLocalItem ? ColorDA.list : ColorDA.listAssets).find(
      e => e.GID == colorID || e.ListID?.includes(colorID)
    )
    if (colorSkin) {
  ...
  if (item.StyleItem.DecorationItem?.BorderID) {
    ...
    let borderSkin = (isLocalItem ? BorderDA.list : BorderDA.listAssets).find(
      e => e.GID == borderID || e.ListID?.includes(borderID)
    )
    if (borderSkin) {
  ...
  if (item.StyleItem.DecorationItem?.EffectID) {
    ...
    let effectSkin = (isLocalItem ? EffectDA.list : EffectDA.listAssets).find(
      e => e.GID == effectID || e.ListID?.includes(effectID)
    )
    if (effectSkin) {
  ...
  if (
    item.StyleItem.TextStyleID ||
    item.CateID === EnumCate.text ||
    item.CateID === EnumCate.textformfield
  ) {
    let typoID = item.StyleItem.TextStyleID
    if (typoID) {
      let typoSkin = (isLocalItem ? TypoDA.list : TypoDA.listAssets).find(
        e => e.GID == typoID || e.ListID?.includes(typoID)
      )
      if (typoSkin) {
...
function initPositionStyle (item) {
  if (item.StyleItem) {
    if (item.ParentID === wbase_parentID)
      item.StyleItem.PositionItem.FixPosition = false
    if (item.StyleItem.PositionItem.FixPosition)
      $(item.value).addClass('fixed-position')
    ...
    if (item.ParentID === wbase_parentID) {
      ...
      item.value.setAttribute(
        'constx',
        item.StyleItem.PositionItem.ConstraintsX
      )
      item.value.setAttribute(
        'consty',
        item.StyleItem.PositionItem.ConstraintsY
      )
      ...
      switch (item.StyleItem.PositionItem.ConstraintsY) {
        ...
        case Constraints.center:
          ...
          if (item.StyleItem.PositionItem.ConstraintsX === Constraints.center)
            item.value.style.transform = 'translateX(-50%) translateY(-50%)'
...
function initWbaseStyle (item) {
  if (item.StyleItem.FrameItem) {
    ...
    if (EnumCate.no_child_component.every(cate => item.CateID != cate)) {
      item.value.style.overflow =
        item.StyleItem.FrameItem.IsClip === true ? 'hidden' : null
    }
  }
  if (item.StyleItem.DecorationItem) {
    if (item.StyleItem.DecorationItem.ColorValue) {
      ...
      if (background.match(hexRegex) && item.CateID !== EnumCate.svg) {
        if (item.StyleItem.DecorationItem.ColorID) {
          switch (item.CateID) {
            case EnumCate.w_switch:
              item.value.style.setProperty(
                '--checked-color',
                `var(--background-color-${item.StyleItem.DecorationItem.ColorID})`
              )
              break
            case EnumCate.checkbox:
              item.value.style.setProperty(
    ...
    if (item.StyleItem.DecorationItem.BorderItem) {
      if (item.StyleItem.DecorationItem.BorderItem.IsStyle) {
        ...
        let listWidth =
          item.StyleItem.DecorationItem.BorderItem.Width.split(' ')
        item.value.style.borderWidth = `${listWidth[0]}px ${listWidth[1]}px ${listWidth[2]}px ${listWidth[3]}px`
        item.value.style.borderStyle =
          item.StyleItem.DecorationItem.BorderItem.BorderStyle
        item.value.style.borderColor = `#${item.StyleItem.DecorationItem.BorderItem.ColorValue}`
      } else {
        item.StyleItem.DecorationItem.BorderID = null
        item.StyleItem.DecorationItem.BorderItem = null
    ...
    if (item.StyleItem.DecorationItem.EffectItem) {
      if (item.StyleItem.DecorationItem.EffectItem.IsStyle) {
        if (
          item.StyleItem.DecorationItem.EffectItem.Type == ShadowType.layer_blur
        ) {
        ...
        item.value.style.boxShadow = `${
          item.StyleItem.DecorationItem.EffectItem.OffsetX
        }px ${item.StyleItem.DecorationItem.EffectItem.OffsetY}px ${
          item.StyleItem.DecorationItem.EffectItem.BlurRadius
        }px ${
          item.StyleItem.DecorationItem.EffectItem.SpreadRadius
        }px #${effect_color}${
          item.StyleItem.DecorationItem.EffectItem.Type == ShadowType.inner
            ? ' inset'
            : ''
        }`
      }
  ...
  if (item.StyleItem.TypoStyleItem && item.CateID != EnumCate.textformfield) {
    ...
    item.value.style.textOverflow =
      item.StyleItem.TypoStyleItem.TextOverflow?.replace('TextOverflow.', '') ??
      'ellipsis'
  }
}
...
function handleStyleSize (item) {
  if (isNaN(item.StyleItem.FrameItem.Width)) {
    ...
    if (
      [Constraints.left, Constraints.right, Constraints.center].some(
        constX => item.StyleItem.PositionItem.ConstraintsX === constX
      )
    ) {
  ...
  if (isNaN(item.StyleItem.FrameItem.Height)) {
    ...
    if (
      [Constraints.top, Constraints.bottom, Constraints.center].some(
        constY => item.StyleItem.PositionItem.ConstraintsY === constY
      )
    ) {
      if (item.CateID === EnumCate.tree) {
        item.value.style.setProperty(
          '--height',
          `${item.StyleItem.FrameItem.Height}px`
        )
        item.value.style.height = `${
          item.StyleItem.FrameItem.Height *
          ([...item.value.querySelectorAll('.w-tree')].filter(
            wtree => wtree.offsetHeight > 0
          ).length +
            1)
        }px`
...
function handleStyleLayout (wbaseItem, onlyPadding = false) {
  ...
  if (wbaseItem.StyleItem) {
    if (wbaseItem.CateID === EnumCate.table) {
      wbaseItem.value.style.setProperty(
        '--text-align',
        wMainAxis(wbaseItem.WAutolayoutItem.Alignment)
      )
      wbaseItem.value.style.setProperty(
        '--vertical-align',
        wCrossAxis(wbaseItem.WAutolayoutItem.Alignment)
      )
    } else if (wbaseItem.CateID === EnumCate.carousel) {
      wbaseItem.value.style.alignItems = wCrossAxis(
        wbaseItem.WAutolayoutItem.Alignment,
        true
      )
      wbaseItem.value.style.setProperty(
        '--justify-content',
        wMainAxis(wbaseItem.WAutolayoutItem.Alignment, true)
      )
      wbaseItem.value.style.setProperty(
        '--gap',
        `${wbaseItem.WAutolayoutItem.ChildSpace}px`
      )
    } else {
      ...
      if (wbaseItem.WAutolayoutItem.IsWrap)
        wbaseItem.value.setAttribute('wrap', 'wrap')
      else wbaseItem.value.removeAttribute('wrap')
      wbaseItem.value.style.setProperty(
        '--child-space',
        `${wbaseItem.WAutolayoutItem.ChildSpace}px`
      )
      wbaseItem.value.style.setProperty(
        '--run-space',
        `${wbaseItem.WAutolayoutItem.RunSpace}px`
      )
      wbaseItem.value.style.setProperty(
        '--main-axis-align',
        wMainAxis(wbaseItem.WAutolayoutItem.Alignment, isRow)
      )
      wbaseItem.value.style.setProperty(
        '--cross-axis-align',
