...
//         let transformX = '0'
//         switch (window.getComputedStyle(item.value).textAlign) {
...
//         }
//         let transformY = '0'
//         switch (window.getComputedStyle(item.value).alignItems) {
//           case TextAlignVertical.center:
//             transformY = '-50%'
//             var thisComputeStyle = window.getComputedStyle(item.value)
//             item.value.style.top =
//               parseFloat(thisComputeStyle.top.replace('px')) +
//               item.value.offsetHeight / 2 +
//               'px'
...
//         } else {
//           try {
//             let textChild = [...contentSpan.childNodes].find(
//               text => text.nodeValue == inputAction.focusNode
//             )
//             if (textChild) {
...
//       if (
//         !window
//           .getComputedStyle(item.value.parentElement)
...
//         action_list[action_index].selected[0].inputActionIndex =
//           inputActionIndex
//       }
//       if (this.innerHTML == '') {
        ...
        Math.max(
          ...textNodes.map(
            textNode =>
              caclTextSize(
                textNode.data,
                `${node.style.fontWeight} ${node.style.fontSize} ${node.style.fontFamily}`
              ).width
          )
        ) +
        Math.max(...textNodes.map(textNode => textNode.data.length)) *
          parseFloat(node.style.letterSpacing.replace('px'))
    } else {
