...
function createNewSkin() {
    let input_new_skin_name = document.getElementById(
        "input_new_skin_name"
    );
    let new_skin;
    ...
    switch (parseInt(popup_cate)) {
        ...
        case EnumCate.typography:
            ...
            new_skin = {
                ...
                FontWeight: select_typo.FontWeight.toString().replace(
                    "FontWeight.w",
                    ""
                ),
                IsStyle: true,
...
for (let i = 0; i < list_cate.length; i++) {
    ...
    switch (list_cate[i]) {
        ...
        case EnumCate.effect:
            ...
            demo_skin.className = "box20";
            demo_skin.src = "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/effect-settings.svg";
            break;
        default:
            break;
    }
...
let list_tool = [
    {
        ...
        scr: "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/move.svg",
    },
    {
        message: ToolState.hand_tool,
        scr: "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/hand.svg",
    },
];

let logo_wini = document.createElement("img");
logo_wini.src = "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/logo_wini.svg";
...
function update_UI_owner(owner_item) {
    ...
    $('.wmember-container[data-customer="' + owner_item.CustomerID + '"]').find(".fa-chevron-down").hide();
}
...
button_share.onclick = function () {
    ...
    for (let customer of PageDA.customerList) {
        list_customer +=
            '<div data-id="' + customer.ID + '" data-customer="' + customer.CustomerID + '" class="wmember-container row">' +
            '    <div class="center box32"><i class="fa-solid fa-user-circle text-body"></i></div>' +
            '    <span class="regular11 text-title space">' +
            '        <span>' + `${customer?.CustomerName ?? "Anonymus"}` + '</span > ' +
            '        <span class="indentify regular11 text-subtitle ' + `${userItem.ID == customer.CustomerID ? "" : " hide"}` + '">(You)</span>' +
            '    </span>' +
            '    <button class="edit-member-permission wbutton-permission button-transparent overlay" type="button" >' +
            '        <span>' +
            '            <span class="user_permission">' + EnumPermission.get_namePermission(customer.Permission) + '</span>' +
            '            <i class="fa-solid fa-chevron-down fa-xs"></i>' +
            '        </span>' +
            '    </button>' +
            '    <div class="wpopup-edit-permisson permission_popup">' +
            '        <div class="owner ' + `${p_user.Permission == EnumPermission.owner ? "" : "hide "}` + 'permission-option regular11 text-white">Owner</div>' +
            '        <div class="edit ' + `${p_user.Permission != EnumPermission.viewer && (customer.CustomerID == userItem.ID || p_user.Permission < customer.Permission) ? "" : "hide "}` + 'permission-option regular11 text-white">Can edit</div>' +
            '        <div class="view permission-option regular11 text-white">Can view</div>' +
            '        <div class="remove ' + `${p_user.Permission == EnumPermission.owner ? "" : "hide "}` + 'permission-option regular11 text-white border-top-light">remove</div>' +
            '        <div class="leave ' + `${p_user.Permission != EnumPermission.owner && customer.CustomerID == userItem.ID ? "" : "hide "}` + 'permission-option regular11 text-white border-top-light">Leave</div>' +
            '    </div>' +
            '</div>';
    }
    list_customer += '</div>';
    ...
    $("#invite_container").validate({
        rules: {
            Email: {
                required: true, email: true, minlength: 8
            }
        }, messages: {
            Email: {
    ...
    $('body').on('click', '.wpopup-edit-permisson .permission-option', function (ev) {
        if ($(this).hasClass("owner")) {
            ...
            $('.wmember-container[data-customer="' + userItem.ID + '"]').find('.user_permission').text("Can edit");
            ...
            $('.wmember-container[data-customer="' + p_user.CustomerID + '"]').find(".fa-chevron-down").show();

            $('.wmember-container[data-customer="' + selected_cus.CustomerID + '"]').css("pointer-events", "none");
            $('.wmember-container[data-customer="' + selected_cus.CustomerID + '"]').find(".fa-chevron-down").hide();
        }
...
button_play.src = "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/play.svg";
...
function customerList() {
    ...
    customer_circle.replaceChildren(...PageDA.customerList.slice(0, 3).map(customerItem => {
    ...
    if (PageDA.customerList.length > 3) {
        ...
        let tooltip = document.createElement("span");
        tooltip.innerText = PageDA.customerList.slice(3).map(customerItem => {
            if (!customerItem.CustomerName && customerItem.CustomerID === user.ID)
                customerItem.CustomerName = user.Email;
            const randomColor = Ultis.generateRandomColor();
            customerItem.color = randomColor;
...
function permissionTool() {
    if (PageDA.enableEdit) {
        list_tool.splice(1, 0, ...[
            {
                message: ToolState.frame,
                expand: function () {
                    console.log("frame");
                },
                scr: "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/frame.svg",
            },
            {
                ...
                scr: "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/rectangle.svg",
            },
            {
                ...
                scr: "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/base_component.svg",
            },
            {
                message: ToolState.text,
                scr: "https://cdn.jsdelivr.net/gh/WiniGit/goline@c082ccf/lib/assets/text.svg",
            }
        ]);
    } else {
        $('.f12-container').css('pointer-event', 'none');
    }
    create_obj_tool.replaceChildren(logo_wini, ...list_tool.map(wTool => {
