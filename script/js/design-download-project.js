function get_listItemInside (pageID) {
  let list = wbase_list.filter(
    a => a.ListID.includes(pageID) || a.GID === pageID
  )
  return list
}
...
async function push_dataProject () {
  ...
  var list_page = wbase_list.filter(
    e =>
      e.ParentID === wbase_parentID &&
      EnumCate.extend_frame.some(ct => ct === e.CateID)
  )
  let replaceSkinRoot = []
  let colorReplace = ColorDA.list.map(skin => {
    ...
    return {
      ...
      Name:
        (cateName
          ? `${Ultis.toSlug(cateName.replace(spChaRegex, '-'))}-`
          : '') + Ultis.toSlug(skin.Name.replace(spChaRegex, '-'))
    }
  })
  ...
  list_page = list_page.map(wb => {
    ...
    let cssString = ''
    ;[cloneValue, ...cloneValue.querySelectorAll('.wbaseItem-value')].forEach(
      wbValue => {
        ...
        switch (parseInt(wbValue.getAttribute('cateid'))) {
          ...
          case EnumCate.textformfield:
            ...
            if (wbItem) {
              if (wbItem.JsonItem.LabelText?.length > 0)
                wbValue.setAttribute('placeholder', wbItem.JsonItem.HintText)
              else
                wbValue.querySelector('input').placeholder =
                  wbItem.JsonItem.HintText
              if (wbItem.JsonItem.JsonVadidate?.length) {
                wbValue.setAttribute(
                  'validate',
                  JSON.stringify(wbItem.JsonItem.JsonVadidate)
                )
              }
            }
            break
          ...
          case EnumCate.tree:
            wbValue
              .querySelectorAll('.w-tree')
              .forEach(wtree => (wtree.style.pointerEvents = null))
            break
          default:
            break
        }
        ...
        if (thisCssText?.length > 0) {
          if (
            wbValue.getAttribute('iswini') === 'true' ||
            wbValue.getAttribute('isinstance') === 'true'
          ) {
            let cssRule = StyleDA.docStyleSheets.find(e =>
              [...divSection.querySelectorAll(e.selectorText)].includes(
                document.getElementById(wbValue.id)
              )
            )
            let propList = thisCssText
              .split(';')
              .map(e => e.split(':').shift().trim())
              .filter(e => e.length > 0)
            for (const propStyle of propList) {
              if (cssRule.style[propStyle] === wbValue.style[propStyle]) {
                wbValue.style[propStyle] = null
              }
            }
            thisCssText = wbValue.style.cssText
          }
          thisCssText = thisCssText.replace(
            uuid4Regex,
            match =>
              replaceSkinRoot.find(skin => skin.GID === match)?.Name ?? match
          )
        ...
        wbValue.removeAttribute('isinstance')
  ...
  let skinRoot = `:root {
    ${document.documentElement.style.cssText.replace(
      uuid4Regex,
  ...
  for (let page of list_page) {
    ...
    for (let witem of list_itemShow) {
      if (witem.JsonEventItem) {
        ...
        if (router_item) {
          ...
          if (page.id !== witem.GID)
            clickElement = page.querySelector(
              `.wbaseItem-value[id="${witem.GID}"]`
            )
          ...
          // } else {
          new_url = `/${
            RouterDA.list.find(e => e.Id == router_item.RouterID)?.Route ?? ''
          }`
          // }
          if (new_url.length > 0) {
            page_script +=
      ...
      if (witem.PrototypeID != null) {
        let nextPagePrototype = list_page.find(e =>
          e.classList.contains(witem.PrototypeID)
        )
        if (nextPagePrototype) {
          ...
          if (page.id !== witem.GID)
            clickElement = page.querySelector(
              `.wbaseItem-value[class*="${witem.GID}"]`
            )
          ...
          if (new_url.length > 0) {
            page_script +=
              '<script>' +
              '    $("body").on("click", `.wbaseItem-value[class*="' +
              witem.GID +
              '"]`, function (ev) {' +
              '        location.href = "' +
              new_url +
    ...
    await $.post(
      '/view/build',
      {
        ...
        Type: 1,
        Code: ProjectDA.obj.Code.toLowerCase(),
        Item: `<link rel="stylesheet" href="/Styles/${Ultis.toSlug(
          page.Name
        )}.css" />${page.outerHTML + page_script}`
          .replace(/&/g, '&amp;')
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;')
          .replace(/"/g, '&quot;')
...
$('body').on(
  'click',
  '.download-project:not(".downloading")',
  async function () {
    $('.download-project').addClass('downloading')
    $('.download-project span').html(
      'Download... <i class="fa-solid fa-download fa-sm"></i>'
    )

    $('.download-project i').toggleClass('fa-spinner fa-download')
    $('.download-project i').addClass('fa-spin')

    let list_page = wbase_list.filter(
      e =>
        e.ParentID === wbase_parentID &&
        EnumCate.extend_frame.some(ct => ct === e.CateID)
    )
        ...
        router = [
          {
            ...
            Route: '',
            Sort: 0,
            PageName: Ultis.toSlug(list_page[list_page.length - 1].Name)
          }
        ]
      }
      await $.post(
...
try {
  ...
  $('body').on('click', '.btn-play', async function () {
    ...
    if (
      ...
      list_page.some(e => e.ID == selected_list[0].ID)
    ) {
      router = [
        {
          Id: 0,
          ...
          Route: '',
          Sort: 0,
          PageName: Ultis.toSlug(selected_list[0].Name)
        }
      ]
    } else {
      if (ProjectDA.obj.RouterJson != null) {
    ...
    await $.post(
      ...
      function (data) {
        ...
        $('.btn-play').html(
          `<img src="https://cdn.jsdelivr.net/gh/WiniGit/goline@5da339b/lib/assets/play.svg" class="btn-play">`
        )
      }
    )
  })
