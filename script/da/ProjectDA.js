class ProjectDA {
    ...
    static initLayoutResponsive() {
        fetch("https://cdn.jsdelivr.net/gh/WiniGit/goline@7925dd0/css/css_Responsive.txt")
    ...
    static create_UI_Project(item) {
        ...
        if (HomeDA.sort_project == 1) {
            pro =
                '<div class="disable-text-select project-card-container ' + `${ProjectDA.selected?.ID == item.ID ? "selected" : ""}` + '">' +
                ...
                '            <img src="https://cdn.jsdelivr.net/gh/WiniGit/goline@7925dd0/lib/assets/pen.svg" class="box32">' +
                '                <div class="col">' +
            ...
            pro =
                '<div class="disable-text-select project-tile elevation4 row ' + `${ProjectDA.selected?.ID == item.ID ? "selected" : ""}` + '" data-id="' + item.ID + '">' +
                '    <div class="block1 row">' +
                '        <button class="button-transparent box32"><i class="fa-regular fa-star"></i></button>' +
                '        <input type="text" class="button-text-5 text-title" value="' + item.Name + '" disabled>' +
                '    </div>' +
                '    <div class="block2">' +
                '        <span class="regular11 text-subtitle">Editor ' + Ultis.getTimeEdit(item.DateUpdate) + ' ago</span>' +
                '    </div>' +
                '    <div class="block3 row">' +
                '        <div class="box12 circular background-grey3"></div>' +
                '        <div class="box12 circular background-grey3"></div>' +
                '    </div>' +
                '</div>';
        }
