class StyleDA {
  ...
  static mergeSkin(styleInitItem) {
    if (styleInitItem.ColorItems?.length > 0) {
      ...
      wbase_list
        .filter((wbaseItem) => listMergeColorID.some((id) => wbaseItem.StyleItem.DecorationItem?.ColorID == id))
        .forEach((wbaseItem) => {
    ...
    if (styleInitItem.TextStyleItems?.length > 0) {
      ...
      wbase_list
        .filter((wbaseItem) => listMergeTypoID.some((id) => wbaseItem.StyleItem.TextStyleID == id))
        .forEach((wbaseItem) => {
    ...
    if (styleInitItem.BorderItems?.length > 0) {
      ...
      wbase_list
        .filter((wbaseItem) => listMergeBorderID.some((id) => wbaseItem.StyleItem.DecorationItem?.BorderID == id))
        .forEach((wbaseItem) => {
    ...
    if (styleInitItem.EffectItems?.length > 0) {
      ...
      wbase_list
        .filter((wbaseItem) => listMergeEffectID.some((id) => wbaseItem.StyleItem.DecorationItem?.EffectID == id))
        .forEach((wbaseItem) => {
...
class TypoDA {
  ...
  static delete(textStyleItem) {
    wbase_list
      .filter((e) => e.StyleItem.TextStyleID === textStyleItem.GID)
      .forEach((wb) => {
        let defaultTypo = {
          GID: 0,
          FontSize: 14,
...
class EffectDA {
  ...
  static delete(effectItem) {
    wbase_list
      .filter((e) => e.StyleItem.DecorationItem.EffectID === effectItem.GID)
      .forEach((e) => {
...
class BorderDA {
  ...
  static delete(borderItem) {
    wbase_list
      .filter((e) => e.StyleItem.DecorationItem.BorderID === borderItem.GID)
      .forEach((e) => {
